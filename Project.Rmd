...
This document is the write-up submission for the course Practical Machine Learning by Jeff Leek, PhD, Professor at Johns Hopkins University, Bloomberg School of Public Health. This 4-week course was offered on Coursera
...
```{r}
setwd("/Users/kalpanarupani/Practical_Machine_Learning")  # or any directory of your choice
...
We will split the training set into two for cross validation purposes. We randomly subsample 60% of the set for training purposes (actual model building), while the 40% remainder will be used only for testing, evaluation and accuracy measurement.
```{r}
library(caret)
library(rpart)
library(rpart.plot)
...
confusionMat
```
We have 99.77%  accuracy which totally validates the idea / hypothesis we made to eliminate most variables and use only 9 relatively independent covariates.
```{r}
...
OOS_errRate
```
he out-of-sample error rate is 0.23%.
